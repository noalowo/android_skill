...
- 同步 `develop` / `main` 用 `git pull --ff-only`；失敗代表工作目錄有未提交改動（先 stash）或本地已與遠端分歧，屬後者且確認丟棄後才用 `git fetch origin && git reset --hard origin/<branch>`。feature 分支跟上 develop 用 `git pull --rebase origin develop`。
